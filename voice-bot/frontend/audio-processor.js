class VoiceAudioProcessor extends AudioWorkletProcessor {
        ...
        this.port.onmessage = (event) => {
            ...
            if (data.type === "playback") {
                ...
                this.startBufferSamples = Math.max(
                    0,
                    Math.floor(
                        (this.playbackSampleRate * startBufferMs) / 1000
                    )
                );
                ...
                this.playbackQueueSamples += samples.length;

                if (
                    !this.playbackStarted &&
                    this.playbackQueueSamples >= this.startBufferSamples
                ) {
    ...
    _writePlayback(outputChannel) {
        ...
        while (outputIndex < outputChannel.length) {
            ...
            const available =
                current.length - this.playbackReadOffset;
            const needed = outputChannel.length - outputIndex;
            ...
            this.playbackReadOffset += count;
            outputIndex += count;
            this.playbackQueueSamples -= count;

            if (this.playbackReadOffset >= current.length) {
                this.playbackQueue.shift();
                this.playbackReadOffset = 0;
            }
        }

        if (this.playbackQueueSamples === 0) {
            this.playbackStarted = false;
        }
    }
    ...
    process(inputs, outputs) {
        ...
        if (output && output.length > 0) {
            ...
            if (outputChannel) {
                this._writePlayback(outputChannel);
            }
        }
